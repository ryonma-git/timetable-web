...
### Manus サーバーを開発プレビュー環境として活用する

**背景**  
GitHub Pages は `stable`（検証済み安定版）のみを配信する。  
`main`（最新開発版）を実際のブラウザで動作確認するには、ローカルの開発サーバーを
立ち上げるか、別の公開環境が必要になる。

**Manus サーバーの役割**  
Manus の独自サーバー（`https://timetableapp-ee4m7qag.manus.space`）は、
もともと `stable` 相当のコードで動いていたと考えられる。  
これを **`main` ブランチの最新状態を反映するプレビュー環境** として活用することを
ユーザーが提案している。
...
| URL | ブランチ | 用途 |
|-----|---------|------|
| `https://ryonma-git.github.io/timetable-web/` | `stable` | 配布・エンドユーザー向け |
| `https://timetableapp-ee4m7qag.manus.space` | `main`（最新） | 開発プレビュー・動作確認 |

**Manus への依頼事項**  
Manus が回復した際には、独自サーバーが `main` ブランチの最新を自動または
手動で追従できるよう設定変更を検討してほしい。  
設定変更の可否・方法はユーザーと相談の上で決定すること。
