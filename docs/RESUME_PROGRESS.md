...
## 2026-06-18 深夜3 ブラウザ方式(B)＋カスケード調査の到達点（総155件・13教科）

### カスケード調査の方法論（各社この順で当てる）
①DOCX/XLSXリンクが生HTMLにある → ②計画PDF → ③直HTML表 → ④JS描画（ブラウザ要）。
判定ツール: curlで `href=".xlsx/.docx/.pdf"` と `<table>`＋計画語(時数/単元) をgrep。

### Tier①で取得済みの社（curlで完結）
東書・教育出版・啓林館・開隆堂・光村・大日本（＝主要6社）。

### Tier④（JS描画）はブラウザ方式で取得
- **教育芸術社 音楽1-6年**（`kyogei_ongaku*`）取得成功。手順: ブラウザで `kyogei.co.jp/es/`→「指導計画案」→`/textbook/es/document/`のExcelボタンの href=`/download/<id>/` を read_page で取得（1年26622〜6年26632, 偶数飛び）→ curl（要 referer）。題材Topシートの「題材名(B1)＋扱い時数(N時間)」を解析。標準(68/70/60/60/50/50)と整合。
- **音楽の社間QA**: 教芸が標準と一致。教育出版音楽は随時題材を除外したため過少(44/50/42/47/40/37)。教育出版側は随時題材の時数扱いを将来見直す候補。

### ブラウザ方式で判明した「取得できない/しない」社
- **学校図書**: サイトは https証明書エラー（curlは -k で可）。R6資料は JS製SPA `r6-sho.gakuto-plus.jp/<教科>`、実ファイルは `/wp-content/uploads/r6_<教科>_*.xlsx`。**ただし学校図書は国語を発行していない**（ポータルに算数・理科・生活のみ）。生活は `r6_seikatsu_jou_nenkanhairetsu_2404.xlsx`等あるが**横型ガント表(列DE超)で信頼読み取り困難→見送り**。算数・理科は収録済み。
- **日本文教**: 教科書ページに検討資料・内容解説・趣意書のみ。**年間指導計画を独立ファイルで配布していない**（道徳・図工・算数・生活・書写すべて計画ファイル0）。
- **帝国書院**: 地図帳が主。地図に年間指導計画は無い（本人確認済み）。

### 実質的な到達ライン
13教科を収録、主要教科は3〜6社、技能系は1〜3社。残りは(a)配布なし(日文等)、(b)横型ガント等で誤読リスク、(c)低価値な4社目、のいずれか。クリーンに取れる範囲はほぼ到達。
