...
7. **v76 言語切替の全画面展開**
   v76 では `client/src/contexts/LanguageContext.tsx` を追加し、サイドバーとトップバーの主要文言だけを段階的に英語化した。v77 で週間グリッド、自動復元、変更確認、学期タブまで拡張済み。日本語画面をデフォルトとして維持し、各ダイアログ、トースト、PDF/Excel/ICS内の固定文言は同じ `t()` 方式で順次移行する。
